...
const CustomerListView = ({ values }) => {
  ...
  const columns = [
...

    {
      ...
      cell: row => row.id != null ? (
        <Button color="primary" onClick={() => click(row.id, row.customer_name, row.customer_address, row.customer_phone, row.action , row.customer_reason, row.destination_bank, row.destination_branch, row.remark, row.account_holder, row.beneficiary_name)}>
        Reason
      </Button>
      ) : (
        ''
  ...
  const click = (id, customer_name, customer_address, customer_phone, action, customer_reason, destination_bank, destination_branch, remark, account_holder, benef_name) => {
  ...
  const getUserList = () => {
    ...
    axios
      ...
      .get(`${url}/big_withdrow/all/${page}/${countPerPage}` ,{withCredentials: true})
      .then(res => {
  ...
  return (
    ...
    >
      ...
      {isLoggedIn ?  <Container style={{ marginTop: 2 }}>
      {!dataFetched ? (
   ...
   </Container>  : <Navigate to="/rms2/login" />}
