...
const ClassCourse = ({}) => {
  ...
  const [insClassCourse, setClassCourse] = useState({});
  ...
  const getClassCourse = () => {
    setDataFetchedClass(false);
    axios
      .get(`${url}/class_course_all`, {
        withCredentials: true
      })
      .then(resp => {
        setCourse({});
  ...
  const editClicked = (id, class_id, course_id) => {
    console.log(id);
    updateID = id;
    setValues({
      ...values,
    ...
    >
      {isLoggedIn ? (
        <Container style={{ marginTop: 2 }}>
          ...
          <DataTable
            title={"Assigned Course for a Class"}
            columns={columns}
