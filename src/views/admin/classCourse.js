...
import CreateClassCourse from './createClassCourse';
...
const ClassCourse = ({}) => {
  ...
  const editClicked = (id, class_id, course_id, stream_id, instructor_id) => {
    console.log(id);
    updateID = id;
    setValues({
      ...values,
  ...
  const deleteClassCourse = () => {
    ...
    axios
      .delete(url + '/class_course/' + deleteID, {
        withCredentials: true
      })
      .then(
        res => {
  ...
  const getClassCourseList = () => {
    ...
    axios
      .get(
        `${url}/class_course/${page}/${countPerPage}/${searchInput}/${searchRequested}`,
        {
          withCredentials: true
        }
      )
  ...
  return (
    <Page
      ...
      title="Instructor Class Courses"
      ...
      {isLoggedIn ? (
        <Container style={{ marginTop: 2 }}>
          <Button
            ...
            variant="outlined"
            color="primary"
            onClick={() => createInstructorClass()}
          >
            <PersonAddIcon />
            {'  '} Create Instructor Class Course
          </Button>
                          ...
                          >
                            ...
                            {instructor.map(option => (
                              <option
                                key={option.id}
                                value={option.id}
                                selected={values.instructor_id == option.id}
                              >
          ...
          <DataTable
            title={'Assigned Course and Class for instructor'}
            columns={columns}
            data={classCourse.data}
            highlightOnHover
            pagination
            paginationServer
